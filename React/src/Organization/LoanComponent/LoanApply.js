    ...
    e.preventDefault();
     ...
     const formData = Formvlaues
       console.log('Formvlaues === ', Formvlaues);
        axios.post(`/Cust_Loan_apply/${token}`, formData)
       .then(resp => {
        const data = resp.data.find_Loans;
        setData(data)
...
const userlist = async () => {
        await axios.post(`/Cust_Loan_apply/${token}`)
            .then(resp => {
                const data = resp.data.find_Loans;
                setData(data)
                const total = resp.data.totlaRow
    ...
    const fetchComments = async (page) => {
       ...
       axios.post(`/Cust_Loan_apply/${token}`, senData)
            .then(resp => {
                const data = resp.data.find_Loans;
                setData(data);
            })
...
const handleView = (_id) =>{
    console.log(_id)
    window.location = `/loan-dashboard/${_id}`;
    return false;  
}
...
const handleDigitalId =(_id) =>
{
   ...
   axios.post(`/updateDigitalID/${custID}`)
   .then(resp =>{
    if(resp.status){
        let data = resp.data
        toast.success(data.msg);
                                            ...
                                            <table className="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
                                                ...
                                                <thead>
                                                    ...
                                                    <tr className="fw-bold text-muted th-title">
                                                        ...
                                                        <th className="min-w-125px">Total Interest Amount</th>
                                                ...
                                                {data.map((item)=>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td><span>{item.Interest_Rate}</span> </td>
                                                        <td>{item.EMI}</td>
                                                        <td>{item.Total_Amount}</td>
                                                        <td>{item.Duration_Year}</td>
                                                        <td>{item.Total_Interest_Amount}</td>
                                                        {item.Loan_status == "PENDING" ?
                                                         <><td><span className="badge badge-light-pending fs-5">{item.Loan_status}</span></td>
                                                         </>:
                                                         <><td><span className="badge badge-light-info fs-5">{item.Loan_status}</span></td>
                                                         </> }
