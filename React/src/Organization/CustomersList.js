    ...
    e.preventDefault();
     ...
     const formData = Formvlaues
       console.log('Formvlaues === ', Formvlaues);
        axios.post(`/OrganisationCustomerTest/${orgID}`, formData)
       .then(resp => {
...
const userlist = async () => {
        await axios.post(`/OrganisationCustomerTest/${orgID}`)
            .then(resp => {
                const data = resp.data.filter;
                setData(data)
                const total = resp.data.totlaRow
    ...
    const fetchComments = async (page) => {
       ...
       axios.post(`/OrganisationCustomerTest/${orgID}`, senData)
            .then(resp => {
                const data = resp.data.filter;
                setData(data);
            })
                                            ...
                                            <table className="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
                                                ...
                                                {data.map((item)=>
                                                    ...
                                                    <tr>
                                                        ...
                                                        {item.status == "pending" ?<>
                                                        <td><span className="badge badge-light-pending fs-5">{item.status}</span></td>
                                                        </>:
                                                        <>
                                                        ...
                                                        <td>
                                                             ...
                                                             }
                                                            {
                                                               ...
                                                               <img src="/assets_new/images/suspand.png" width="20px" />
                                                                ...
                                                                <button onClick={(e)=>{handleDigitalId(item._id); }}
                                                                    className="btn btn-icon btn-detail btn-active-color-primary btn-sm me-1 mr-4"
                                                                    data-bs-toggle="tooltip" data-bs-placement="top"
                                                                    data-bs-trigger="hover" title="Digital ID ">
                                                                    <span className="svg-icon svg-icon-3">
