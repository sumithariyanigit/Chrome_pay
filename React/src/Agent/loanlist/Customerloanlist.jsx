...
const customerloanlistapply=()=>{
    ...
    axios.post(`/Cust_apply_Agent_Loans/${token}`)
    .then(resp => {
        let data = resp.data.find_Loans;
        setloanlist(data);

    
        console.log("custer loan apply list ",data);
    ...
    <table className="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                <td> {item.Loan_status=="PENDING"? <span class="badge badge-light-pending fs-5">{item.Loan_status}</span>:<span class="badge badge-light-info fs-5">{item.Loan_status}</span>}    </td> 
                </td>
                <td>
