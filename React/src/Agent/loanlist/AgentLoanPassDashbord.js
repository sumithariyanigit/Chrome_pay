...
function LoanPassDashbord() {
    ...
    const [show_emi ,setshow_emi] = useState(false);
    const [show_emi_list ,setshow_emi_list] = useState([]);
    const [Next_EMI ,setNext_EMI] = useState();
  ...
  const emidetail = async () => {
    ...
    .then(resp =>{
        ...
        setemi(data);
        // console.log('emi data 222222222222 ',data);
    })

  };
  const emimonthsdetail = async () => {
    const {LoanID} = {LoanID : _id}
    axios.post(`/get_next_month_emi/${LoanID}`)
    .then(resp =>{
        let data = resp.data.Next_EMI;
        let data1 = resp.data.Amount;
        setNext_EMI(data);
        setAmount(data1)
        console.log('emi next months  data 222222222222 ',data);
    })
  ...
  const show_user_pay_emi =(e)=>{
   const {LoanID} = {LoanID : _id}
    e.preventDefault();
            const data = new FormData(e.target);
            const Formvlaues = Object.fromEntries(data.entries());
            const formData =  Formvlaues
    axios.post(`/pay_cust_emi/${LoanID}`,formData)
    .then(resp =>{
        let data = resp.data;
        setshow_emi(false);
        setshow_emi_list(data);
        console.log('emilist  data 222222222222 ',data);
        if(resp.status){
        ...
        <div className="col-lg-12" >
                                <div className="card mb-5 mb-xl-10">
                                    ...
                                    <div id="kt_account_settings_profile_details" className="collapse show">
                                        ...
                                        <button type="button" className="btn btn-light btn-active-light-primary me-2" onClick={()=> setshow_emi(true) }>Pay EMI</button>
...
<div className="row tebor25  rounded-50 ">
<div className="col-lg-6 ">
                                                        <div className="row ">
                                                            <label className="col-6 col-form-label required fw-semibold fs-6 text-right mt-1">   NEXT EMI Date </label>
                                                            <div className="col-6 col-form-label fw-semibold fs-6 text-right mt-1 ">
                                                            {Next_EMI}
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div className="col-lg-6">
                                                        <div className="row ">
                                                            <label className="col-4 col-form-label required fw-semibold fs-6 mt-1">    Amount</label>
                                                            <label className="col-4 col-form-label fw-semibold fs-6 mt-1">    ${Amount}</label>
                                                            <div className="col-4 fv-row">
                                                            <span className="btn btn-primary my-2 " onClick={()=> setshow_emi(true) } > Pay EMI</span>
                                                            </div>
                                                        </div>
                                                    </div>
...
<tr className="fw-bold text-muted th-title">
    ...
    {/* <th className="min-w-100px "> <h6>pay</h6></th> */}
...
{/* value={moment(data.End_Date).format("DD/MM/YYYY")} */}
                ...
                {item.status!=="Due"?     <td><span class="badge badge-info fs-4 ">{item.status}</span></td>
                :<td><span class="badge bg-danger fs-4">{item.status}</span></td>}
              {/* <td><span className="ms-1" onClick={()=> setshow_emi(true) } > <i class="far fa-money-bill-alt fs-5"></i></span> </td> */}
...
{/* pay emi selection  */}
<Modal show={show_emi} onHide={() => setshow_emi(false)}>
        <Modal.Header closeButton>
          <Modal.Title>Emi details</Modal.Title>
        </Modal.Header>
        <form onSubmit={show_user_pay_emi}>
        <Modal.Body>
             <div className="col-lg-12 ">
             <label className="mb-2">Pay EMI</label>
                      <input className="form-control" placeholder="Enter your EMI"  type="number" name="Amount"/>
                    </div>
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" type="submit" className="center" >  Pay
          </Button>
        </Modal.Footer>
        </form>
      </Modal>
