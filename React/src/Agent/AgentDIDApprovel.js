...
function AgentDIDApprovel() {
    ...
    const userlist = async () =>{
        ...
        await axios.post(`/AgentAwaiting/${token}`)
        .then(resp=>{
...
const fetchComments = async (page) => {
   ...
   axios.post(`/AgentAwaiting/${agentID}`, senData)
        .then(resp => {
   ...
   const handleVerfiy = (_id) => {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {         
            axios.post(`/customerVerify/${custID}/${agentID}`)
            .then(res => {
                if (res.status) {
                    let data = res.data;
                    if (data.status) { 
...
export default AgentDIDApprovel