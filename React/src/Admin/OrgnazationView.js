...
function OrgnazationView() {
    const { _id } = useParams('');
  
    const [data, setData] = useState([])
    const [transection, setTtransection] = useState([]);
    const [recurring, setrecurring] = useState([]);
    const [remaning_Licenses, setremaning_Licenses] = useState([]);
    ...
    const AddFormData = async (e) => {
        ...
        axios.post(`/OrganisationDash/${orgID}`,formData)
        .then(resp =>{
    ...
    const userlist = async () =>{
        const {orgID} = {orgID: _id}
        await axios.post(`/OrganisationDash/${orgID}`)
        .then(resp =>{
            let data =resp.data.filter;
            setData(data);
            console.log(data)
    ...
    useEffect(() => {
        const orgTransections = async () => {
            const {orgID} = {orgID: _id}
            console.log(orgID)
            let result = await fetch(`/OrganisationTransection/${orgID}`)
            console.log(result)
            result = await result.json();
            const transData = await result.data
            //console.log('result',transData)
            //console.log(ID)
            setTtransection(transData);
    ...
    useEffect(() => {
        const orgTransectionsSection = async () => {
            const {orgID} = {orgID: _id}
            let result = await fetch(`/orgDashSection/${orgID}`)
            console.log(result)
            result = await result.json();
            const transData = await result.data
            console.log('result', transData.name)
            //console.log(ID)
...
useEffect(() => {
    const orgTransectionsSection = async () => {
        const {orgID} = {orgID: _id}
        let result = await fetch(`/viewFee/${orgID}`)
        console.log("===>",result)
        result = await result.json();
        //const transData = await result
        //console.log('View recurring', result.findLicenseFees.recuuringFees)
        const transData = await result.findLicenseFees.recuuringFees
        //console.log("transdata",transData)
        setrecurring(transData);
...
// add new api remaning_Licenses
useEffect(() => {
    const orgTransectionsSection = async () => {
        const {orgID} = {orgID: _id}
        let result = await fetch(`/find_Org_RemainingLicenses/${orgID}`)
        console.log("===>",result)
        result = await result.json();
        //const transData = await result
        console.log('View recurring', result)
        const transData = await result.remaning_Licenses
       // console.log("reminig",transData)
        setremaning_Licenses(transData);
                                        ...
                                        <div className="card card-flush">
                                            ...
                                            <div className="bg_div card-header rounded bgi-no-repeat bgi-size-cover bgi-position-y-top bgi-position-x-center align-items-start h-250px">
                                                ...
                                                <h3 className="card-title align-items-start flex-column text-white pt-15">
                                                    <span className="fw-bold fs-2x mb-3">{section.name}</span>
                                                    ...
                                                    <div className="row g-3 g-lg-6">
                                                        ...
                                                        <div className="col-4">
                                                            ...
                                                            <div className="retative bg-gray-100 ative animestion-bank bg-opacity-70 rounded-2 px-6 py-5">
                                                                ...
                                                                <div className="m-0">
                                                                    <span data-kt-countup="true" data-kt-countup-value={section.totalUser} className="text-gray-700 fw-bolder d-block fs-2qx lh-1 ls-n1 mb-1">
                                                                        {section.totalUser}</span>
...
<div className="retative bg-gray-100 animestion-bank bg-opacity-70 rounded-2 px-6 py-5">
    ...
    <div className="m-0">
        <span data-kt-countup="true" data-kt-countup-value={section.totlaLicense} className="text-gray-700 fw-bolder d-block fs-2qx lh-1 ls-n1 mb-1">
            {section.totlaLicense}</span>
        <span className="text-gray-500 fw-semibold fs-6">
            Total License </span>
    </div>
                                                        ...
                                                        <div className="col-4">
                                                            ...
                                                            <div className="retative bg-gray-100 animestion-bank bg-opacity-70 rounded-2 px-6 py-5">
                                                                ...
                                                                <div className="m-0">
                                                                    <span data-kt-countup="true" data-kt-countup-value={transections} className="text-gray-700  fw-bolder d-block fs-2qx lh-1 ls-n1 mb-1">
                                                                        {transections}</span>
                                                                    <span className="text-gray-500 fw-semibold fs-6">
...
<div className="col-4">
                                                            ...
                                                            <div className=" retative bg-gray-100 animestion-bank bg-opacity-70 rounded-2 px-6 py-5">
                                                                ...
                                                                <div className="m-0">
                                                                    <span data-kt-countup="true" data-kt-countup-value={recurring} className="text-gray-700 fw-bolder d-block fs-2qx lh-1 ls-n1 mb-1">
                                                                    {recurring}</span>
                                                                    <span className="text-gray-500 fw-semibold fs-6">
                                                                    Recurring Fees  </span>
...
<div className="retative bg-gray-100 ative animestion-bank bg-opacity-70 rounded-2">
        ...
        Recurring Fees</span>
                                                        ...
                                                        <div className="col-4">
                                                            ...
                                                            <div className=" retative bg-gray-100 animestion-bank bg-opacity-70 rounded-2 px-6 py-5">
                                                                ...
                                                                <div className="m-0">
                                                                    <span data-kt-countup="true" data-kt-countup-value={remaning_Licenses} className="text-gray-700 fw-bolder d-block fs-2qx lh-1 ls-n1 mb-1">
                                                                        {remaning_Licenses}</span>
                                                                    <span className="text-gray-500 fw-semibold fs-6">
                                                                    Remaning Licenses  </span>
                                                ...
                                                <div className="rounded d-flex flex-column flex-lg-row align-items-lg-center bg-body p-5 w-xxl-850px h-lg-60px me-lg-10 my-5">
                                                    ...
                                                    <div className="row flex-grow-1 mb-5 mb-lg-0">
                                                        ...
                                                        <div className="col-lg-4 d-flex align-items-center mb-3 mb-lg-0">
                                                            ...
                                                            <input type="text" autoComplete="off" className="form-control form-control-flush flex-grow-1" name="name" placeholder="User Name.." />
                                                        ...
                                                        <div className="col-lg-4 d-flex align-items-center mb-5 mb-lg-0">
                                                            ...
                                                            <select className="form-select border-0 flex-grow-1" data-control="select2" data-placeholder="country" data-hide-search="true" name="category">
                                ...
                                <div className="card card-xxl-stretch mb-5 mb-xl-8">
                                    ...
                                    <div className="card-header border-0 pt-5">
                                        <h3 className="card-title align-items-start flex-column">
                                            ...
                                            <span className="text-muted mt-1 fw-semibold fs-7">Over {section.totalUser} Customer</span>
                                        </h3>
...
export default OrgnazationView