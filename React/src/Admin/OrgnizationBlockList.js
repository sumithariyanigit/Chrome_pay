...
function OrgnizationBlockList() {
  ...
  const userlist = async () => {
        await axios.post(`/blockedOrglist`)
            .then(resp => {
...
const fetchComments = async (page) => {
   ...
   axios.post(`/blockedOrglist`, senData)
        .then(resp => {
            const data = resp.data.data;
            setData(data);
        })
...
// }
  ...
  const handledelete = (_id) => {
    Swal.fire({
        title: 'Are you sure?',
        text: "Are you sure you want to Unblocked selected Customer ?",
        icon: 'warning',
        ...
        if (result.isConfirmed) {  
            const {OrganisationID} ={OrganisationID :_id} 
            console.log(OrganisationID);     
            axios.put(`/unsuspend/${OrganisationID}`)
            .then(res => {
                if (res.status) {
                    let data = res.data;
                    if (data.status) { 
                                                        ...
                                                        <td>
                                                            ...
                                                            {/* {item.blocked == 1 ? <> 
                                                               ...
                                                               <img src="/assets_new/images/suspand.png" width="20px" />
                                                                ...
                                                                {orgmenu.unBlockOrganisation == 1 ? <><button onClick = {(e) => { handledelete(item._id) ;}}
                                                                    className="btn btn-icon btn-danger1 btn-active-color-primary btn-sm me-1 mr-4"
                                                                    data-bs-toggle="tooltip" data-bs-placement="top"
...
export default OrgnizationBlockList